...
Cypress.Commands.add('DeleteUser', (RetailerID) => {
    cy.get('#retailer-number-filter').type(RetailerID)
    cy.get('.filter-buttons-width > .row > .mr-2 > .rw-button').click()//search button  
    cy.wait(1000)
    cy.get('a.break-word').eq(0).click()// Select user from list
    cy.get('[ng-if="$ctrl.canUpdateData && $ctrl.canEditAccountDetails"] > a').click({force:true})// click on delete
